...
if ($userId > 0) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM loginsessions WHERE userId = :userId");
            ...
            <table id="example1" class="table table-bordered table-hover">
              ...
              <tbody>
                <?php if (!empty($results)): ?>
                  <?php foreach ($results as $index => $results): 
                    $loggedIn = $results['loggedInAt'];
                    $loggedOut = $results['loggedOutAt'];
                    $loggedInTime = new DateTime($loggedIn);
                    $loggedOutTime = new DateTime($loggedOut);
                    $interval = $loggedInTime->diff($loggedOutTime);
                    ?>
                    
                    <tr>
                      <td><?php echo $index + 1; ?></td>
                      <td><?php echo htmlspecialchars($results['loggedInAt']); ?></td>
                      <td><?php echo htmlspecialchars($results['loggedOutAt']); ?></td>
                      <td><?php echo $interval->format('%H hours %I minutes %S seconds');?></td>
                      <td>...</td>
                    </tr>
                  <?php endforeach; ?>
                <?php else: ?>
